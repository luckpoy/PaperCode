...
@main.command()
def train_ctw():
	from train_ctw import train
	train.main()

@main.command()
def east_train_ctw():
	from other_remade.EAST import main
	main.main()

@main.command()
def single_test():
	from train_ctw import single_test
	single_test.main()

@main.command()
@click.option('-model', '--model', help='Path to Model', required=True)
def test_ctw(model):
	from train_ctw import test
	test.main(model)

@main.command()
@click.option('-model', '--model', help='Path to Model', required=True)
def test_proj(model):
	from train_ctw import sm_train
	sm_train.main(model)
