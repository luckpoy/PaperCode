...
import numpy as np
import cv2
from torch import arange, flip
from .data_manipulation import two_char_bbox_to_affinity
import math
import train_synth.config as cfg

import matplotlib.pyplot as plt 

from train_ctw.config import seg_threshold,seg_ignore,seg_flag
...
def seg_boxes(box,img):
	ph = box[3] - box[0]
	height = int(math.hypot(ph[0],ph[1]))
	pw = box[1] - box[0]
	width = int(math.hypot(pw[0],pw[1]))
	...
	# plt.imsave("debug/erosion.png",erosion)

	arr = [0 for n in range(0, w)]
	for column in range(0, w):    # 遍历每一�? 
		for row in range(0, h):    # 遍历每一�?
			if  dst[row, column] == 1:    # 判断该点是否为黑点，0代表是黑�?
				arr[column] += 1     # 该列的计数器�?1
	x = np.arange(0,w)
	acc = [0 for n in range(0, w)]
	org = acc.copy()
	for i in arange(1,w):
		acc[i] = arr[i] - arr[i-1]
	# plt.plot(x,arr,color = 'black',linewidth=2,label='pixel projection')
	# plt.plot(x,acc,color = 'black',linewidth=1,label='derivative')
	# # plt.plot(x,org,color = 'b',linewidth=1)
	
	index = []
	try: 
		for i in range(int(h*seg_ignore), w - int(h*seg_ignore)):
			if arr[i] <= max(arr)*seg_threshold and arr[i] != 0:
				if acc[i-4]<=0 and acc[i-3]<=0 and acc[i-2]<=0 and acc[i-1]<=0 and acc[i+1]>=0 and acc[i+2]>=0 and acc[i+3]>=0 and acc[i+4]>=0:
					if len(index) == 0 :
						index.append(i)
						# plt.axvline(i,linestyle='--',label='dividing line')
					else:
						if i - index[-1] > h*seg_ignore:
							index.append(i)
	...
	# # plt.ylabel('column')
	# plt.legend()
	# plt.savefig("debug/st/dim.jpg")
	# plt.imsave("debug/st/reimg.png",dst)
	return det_flip, np.array(index)/w
...
def generate_word_bbox(
		...
		# affinity_threshold,
		word_threshold,
		# character_threshold_upper=cfg.threshold_character_upper,
		# affinity_threshold_upper=cfg.threshold_affinity_upper,
		# scaling_affinity=cfg.scale_affinity,
		# scaling_character=cfg.scale_character
	):
	...
	""" labeling method """
	ret, text_score = cv2.threshold(character_heatmap, character_threshold, 1, 0)#阈值处�?
	# plt.imsave("fig.png",text_score)
	# ret, link_score = cv2.threshold(affinity_heatmap, affinity_threshold, 1, 0)

	# text_score_comb = np.clip(text_score + link_score, 0, 1)
	# text_score_comb = np.clip(text_score, 0, 1)#将input保持�?0�?1之间'
	text_score_comb = text_score
	...
	for k in range(1, n_labels):
		...
		try:
			...
			zs,zx,ys,yx=box[0],box[3],box[1],box[2]
			ph,pw= zx-zs,ys-zx
			h,w= int(math.hypot(ph[0],ph[1])),int(math.hypot(pw[0],pw[1])) 
			if (w/h<0.56 or w/h>1.8) and seg_flag:
				filp,seg_points = seg_boxes(box,text_score_comb.astype(np.uint8))
				if len(seg_points) != 0:
					if filp:
						for i in arange(len(seg_points)):
							new_zx = zs*(1-seg_points[i]) + zx*seg_points[i]
							new_yx = ys*(1-seg_points[i]) + yx*seg_points[i]
							new_box= np.array([zs,ys,new_yx,new_zx],dtype=np.float32)
							det.append(new_box)
							zs,ys= new_zx,new_yx
					else:#ready modify error
						for i in arange(len(seg_points)):
							new_ys = zs*(1-seg_points[i]) + ys*seg_points[i]
							new_yx = zx*(1-seg_points[i]) + yx*seg_points[i]
							if i == 0:
								new_zs,new_zx = zs,zx
							new_box= np.array([new_zs,new_ys,new_yx,new_zx],dtype=np.float32)
							det.append(new_box)
							new_zs,new_zx= new_ys,new_yx
						zs, zx =  new_zs,new_zx
					
			det.append(np.array([zs,ys,yx,zx],dtype=np.float32))
	...
	return {
		...
		'characters': 1,#char_contours,
		'affinity': 1#affinity_contours,
	}
...
def generate_word_bbox_batch(
	...
	word_bbox = []
	batch_size = batch_character_heatmap.shape[0]
...
def calculate_batch_fscore(pred, target, text_target, unknown='###', text_pred=None, threshold=0.5):
	...
	for i in range(len(pred)):
		if text_pred is not None:
			...
			if np.zeros(shape=(5,2)).all() == target[i].all():
				f_score += 1
				precision += 1
				recall += 1
			else:
