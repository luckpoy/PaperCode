...
def train(dataloader, loss_criterian, model, optimizer, starting_no, all_loss, all_accuracy,test_dataloader,best):
	def change_lr(no_i):
		for i in config.lr:
			if i == no_i:
	...
	for epoch in range(config.epoch):
		...
		for i, (image, weight, affinity, drawn_image) in enumerate(iterator):
			...
			loss = loss_criterian(output, weight, affinity).mean()/config.optimizer_iteration#+0.1*emb_loss
...
def main():
	if config.model_architecture == 'UNET_ResNet':
		...
		model = CRAFT()
	elif config.model_architecture == 'craft_smalier':
		from .craft_smalier import CRAFT
		model = CRAFT()
